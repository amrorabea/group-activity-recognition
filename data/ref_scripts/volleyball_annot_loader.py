...
dataset_root = '/home/amro/Desktop/group-activity-recognition/data'
...
def print_random_annotation(pkl_path):
    with open(pkl_path, 'rb') as f:
        videos_annot = pickle.load(f)

    # random video
    video_id = random.choice(list(videos_annot.keys()))
    video = videos_annot[video_id]

    # random clip
    clip_id = random.choice(list(video.keys()))
    clip = video[clip_id]

    # random frame
    frame_boxes_dct = clip['frame_boxes_dct']
    frame_id = random.choice(list(frame_boxes_dct.keys()))

    # random player box
    box_info = random.choice(frame_boxes_dct[frame_id])

    print("Video ID:", video_id)
    print("Clip ID:", clip_id)
    print("Clip Category:", clip['category'])
    print("Frame ID:", frame_id)
    print("Player ID:", box_info.player_ID)
    print("Player Category:", box_info.category)
    print("Bounding Box (x1, y1, x2, y2):", box_info.box)
...
if __name__ == '__main__':
    ...
    # print_random_annotation("/home/amro/Desktop/group-activity-recognition/data/annot_all.pkl")
    # vis_clip(annot_file, clip_dir_path)
    create_pkl_version()